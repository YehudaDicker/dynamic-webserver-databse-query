 ...
 * move on.
 */
static int handle_file_request(FILE *mdb_r, FILE *mdb_w, const char *web_root, const char *request_uri, FILE *clnt_w)
{
    /*
    ...
    */
    ...
    if (strstr(request_uri, "mdb-lookup")){

        /*
        mdb_r = fdopen(servMdb_fd, "rb");
        if (mdb_r == NULL)
            die("fdopen");

        mdb_w = fdopen(dup(servMdb_fd), "wb");
        if (mdb_w == NULL)
            die("fdopen");
        ...
        if (strcmp(request_uri, "/mdb-lookup") == 0 || strcmp(request_uri, "/mdb-lookup?") == 0){ 
            status_code = 200; // "OK"
        ...
        else if (strstr(request_uri, "?key=")){
            search_key = strstr(request_uri, "?key=");
            search_key+=5;
            ...
            */
        ...
        else if (strstr(request_uri, "/mdb-lookup?")){
            status_code = 200; // "OK"
            if (send_status_line(clnt_w, status_code) < 0 || send_blank_line(clnt_w) < 0) {
                perror("send");
                goto cleanup;
            ...
            fprintf(stderr, "In last else if\n");
            fprintf(clnt_w, "%s\n", form);
            goto cleanup;
        }
        ...
        const char *table_bottom = "</table>\n"
            "</body></html>\n";


        // fprintf(stderr, "1");
        //fprintf(clnt_w, "%s", table);
        ...
        while((null = fgets(get_buf, sizeof(get_buf), mdb_r)) != NULL){
            ...
            if (strcmp(get_buf, "\n") == 0){
                fprintf(clnt_w, "%s", table_bottom);
                break;
            }       
            else{ 

                if (row_num % 2 == 0){
                    fprintf(clnt_w, "%s", "<tr><td bgcolor=yellow>\n");
                }
                else
                {
                    fprintf(clnt_w, "%s", "<tr><td>\n");
                }
                fprintf(clnt_w, "%s\n", get_buf);
                row_num++;
            }
...
void handle_client(const char *web_root, int clnt_fd, const char *clnt_ip, FILE *mdb_r, FILE *mdb_w)
{
    ...
    status_code = handle_file_request(mdb_r, mdb_w, web_root, request_uri, clnt_w);
    //   goto terminate_connection;
...
int main(int argc, char *argv[])
{
    ...
    /*
     * Construct server socket to listen on mdb_port
     */

    struct addrinfo mdb, *mdb_info;
    memset(&mdb, 0, sizeof(mdb));
    mdb.ai_family = AF_INET;       // Only accept IPv4 addresses
    mdb.ai_socktype = SOCK_STREAM; // stream socket for TCP connections
    mdb.ai_protocol = IPPROTO_TCP; // TCP protocol

    int mdb_addr_err;
    if ((mdb_addr_err = getaddrinfo(mdb_host, mdb_port, &mdb, &mdb_info)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(mdb_addr_err));
        exit(1);
    }

    int servMdb_fd = socket(mdb_info->ai_family, mdb_info->ai_socktype, mdb_info->ai_protocol);
    if (servMdb_fd < 0)
        die("socket");

    if(connect(servMdb_fd, mdb_info->ai_addr, mdb_info->ai_addrlen) < 0)
        die("connect");
   ...
   FILE *mdb_r = fdopen(servMdb_fd, "rb");
        if (mdb_r == NULL)
            die("fdopen");

        FILE *mdb_w = fdopen(dup(servMdb_fd), "wb");
        if (mdb_w == NULL)
            die("fdopen");
